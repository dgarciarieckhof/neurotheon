...
class TurretEnv(gym.Env):
        ...
        if self.steps % self.ally_move_every == 0:
            ...
            for e in self.entities:
                if e["type"] != "ally":
                    continue
                e["pos"] = np.clip(
                    e["pos"] + self.rng.choice(dirs), 0, self.gs - 1)
        ...
        if action:
            ...
            while True:
                ...
                if idx is not None:
                    ...
                    if e["type"] == "enemy":
                        reward += 1;  info["hit"] = "enemy"
                    elif e["type"] == "ally":
                        reward -= 2;  info["hit"] = "ally"
                    else:
                        reward -= .5; info["hit"] = "neutral"
                    break
        else:
            reward -= 0.02      # idle
    ...
    def render(self):  # viewer handles graphics
        pass